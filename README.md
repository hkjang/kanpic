...
가이드 문서(`docs/USER_GUIDE.md`, `docs/ADMIN_GUIDE.md`)의 화면 캡처는 실제로 띄운 서버를 찍은 것입니다. 화면이 바뀌면 버려도 되는 로컬 배포에 대고 다시 찍습니다. 이 스크립트는 워크북·사용자·부서·API 키를 만들어 넣지만 전역 설정은 읽기만 합니다.

```bash
cd web && KANPIC_GUIDE_BASE_URL=http://localhost:8080 \
  KANPIC_GUIDE_ADMIN_ID=... KANPIC_GUIDE_ADMIN_PASSWORD=... \
  node e2e/guide-screenshots.mjs   # docs/images/guide/*.png
```
