...
수식 엔진은 수학·통계·재무·텍스트·날짜·논리·조회·배열 205개 함수를 계산합니다. `=`를 치고 이름을 몇 글자 입력하면 후보가 뜨고 `Tab`으로 넣으면 괄호까지 열리며, 괄호 안에서는 지금 입력 중인 인수가 표시된 사용법이 따라옵니다. `=SUM(A:A)`처럼 열이나 행 전체를 가리킬 수 있고 계산은 실제로 값이 있는 범위까지만 이뤄집니다. 행이나 열을 넣고 지우면 수식 참조는 물론 병합·조건부 서식·데이터 유효성·필터 뷰·차트 원본·이름 지정 범위·댓글 위치까지 함께 조정되며, 참조하던 셀이 사라진 자리만 `#REF!`로 남습니다.
