...
편집기의 상단 메뉴 막대(파일·수정·보기·삽입·서식·데이터·도구·도움말)와 셀·행 머리글·열 머리글·모서리·시트 탭·차트·홈 화면 워크북 카드의 컨텍스트 메뉴는 같은 명령 집합을 제공합니다. 모든 컨텍스트 메뉴는 마우스 오른쪽 클릭으로 열리고 방향키·`Enter`·`Esc`로 조작합니다. 머리글을 클릭하거나 드래그해 행과 열 전체를 선택하고, 이미 선택된 머리글을 다시 잡아 끌면 그 행·열을 통째로 다른 자리로 옮깁니다(놓일 자리에 안내선이 표시되고 수식 참조가 새 위치를 따라갑니다). 머리글 경계를 드래그하면 안내선과 픽셀 값을 보며 크기를 조절하며, 더블클릭하면 셀 내용에 맞춰 자동으로 맞춥니다. 여러 행·열을 선택하면 삽입·삭제·숨기기·크기 조절·그룹화가 한 번에 적용됩니다. 그룹화한 행과 열은 머리글 옆 아웃라인 여백의 `−`·`+` 상자로 접고 펴며 8단계까지 중첩됩니다. 셀에는 마우스를 올렸을 때 보이는 메모를 달 수 있고, 메모는 댓글과 달리 답글 없이 셀에 붙어 함께 움직입니다. 데이터 검증에 체크박스 유형이 있어 클릭이나 스페이스바로 켜고 끄며 저장 값은 `TRUE`/`FALSE` 또는 원하는 두 값입니다. **데이터 › 열 통계** 는 선택한 열의 값 개수·고유값·숫자 요약과 분포, 자주 나오는 값을 오른쪽 패널에 보여 줍니다. 필터 뷰가 켜져 있으면 열 머리글의 깔때기 버튼에서 값 목록을 체크해 바로 거르고 정렬합니다. **데이터 › 슬라이서 추가** 는 그 값 목록을 시트 위 카드로 띄워, 필터 메뉴를 모르는 사람도 버튼만 눌러 거르게 합니다. 슬라이서는 필터 보기를 조작하므로 두 경로의 상태가 항상 같고, 카드 위치·크기는 시트 레이아웃에 저장됩니다. `Alt + Enter`로 셀 안에서 줄 바꾸기, `Ctrl/⌘ + \``로 수식 보기, `Alt + =`로 자동 합계, `Ctrl/⌘ + ;`로 오늘 날짜, `Ctrl/⌘ + Alt + =`/`-`로 행·열 삽입·삭제, `Ctrl/⌘ + PageUp`/`PageDown`으로 시트 전환을 사용할 수 있고 전체 목록은 `Ctrl/⌘ + /`에서 확인합니다. 파일 메뉴는 새 워크북·사본 만들기·이름 변경·인쇄(`Ctrl/⌘ + P`)·휴지통 이동을, 수정 메뉴는 값·서식·행열 바꿔 붙여넣기를, 보기 메뉴는 눈금선과 전체 화면을, 삽입 메뉴는 함수와 링크를, 서식 메뉴는 글꼴·크기·텍스트 색·채우기 색·회전·테이블 서식·테두리를(툴바의 붓 아이콘으로 서식만 다른 범위에 복사하고 두 번 클릭하면 연속 적용), 데이터 메뉴는 선택 열 기준 정렬과 중복 항목 삭제·공백 제거·텍스트를 열로 분할을 제공합니다. 도움말 메뉴의 **함수 목록** 은 서버의 `GET /api/v1/formula/functions` 카탈로그를 그대로 보여 줍니다.
...
도구 모음의 **피벗** 버튼에서는 선택 범위를 행·열로 그룹화하고 합계·평균·개수·최소·최대 집계, 필터, 날짜·숫자·사용자 그룹과 계산 필드를 구성할 수 있습니다. 피벗 정의와 수동 갱신 캐시는 PostgreSQL에 저장되며 자동/수동 새로 고침, 전체 합계, 집계 셀의 원본 행 드릴다운, 구조 변경·복제·버전 복원을 지원합니다. REST `/api/v1/workbooks/{workbookId}/pivots`, `/api/v1/pivots/{pivotId}`와 `spreadsheet.pivot.*` MCP 도구는 동일한 권한·검증 계약을 사용합니다.
