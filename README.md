...
편집기의 상단 메뉴 막대(파일·수정·보기·삽입·서식·데이터·도구·도움말)와 셀·행 머리글·열 머리글·모서리·시트 탭·차트·홈 화면 워크북 카드의 컨텍스트 메뉴는 같은 명령 집합을 제공합니다. 모든 컨텍스트 메뉴는 마우스 오른쪽 클릭으로 열리고 방향키·`Enter`·`Esc`로 조작합니다. 머리글을 클릭하거나 드래그해 행과 열 전체를 선택하고, 머리글 경계를 드래그하면 안내선과 픽셀 값을 보며 크기를 조절하며, 더블클릭하면 셀 내용에 맞춰 자동으로 맞춥니다. 여러 행·열을 선택하면 삽입·삭제·숨기기·크기 조절·그룹화가 한 번에 적용됩니다. 그룹화한 행과 열은 머리글 옆 아웃라인 여백의 `−`·`+` 상자로 접고 펴며 8단계까지 중첩됩니다. 셀에는 마우스를 올렸을 때 보이는 메모를 달 수 있고, 메모는 댓글과 달리 답글 없이 셀에 붙어 함께 움직입니다. 데이터 검증에 체크박스 유형이 있어 클릭이나 스페이스바로 켜고 끄며 저장 값은 `TRUE`/`FALSE` 또는 원하는 두 값입니다. `Ctrl/⌘ + \``로 수식 보기, `Alt + =`로 자동 합계, `Ctrl/⌘ + ;`로 오늘 날짜, `Ctrl/⌘ + Alt + =`/`-`로 행·열 삽입·삭제, `Ctrl/⌘ + PageUp`/`PageDown`으로 시트 전환을 사용할 수 있고 전체 목록은 `Ctrl/⌘ + /`에서 확인합니다. 파일 메뉴는 새 워크북·사본 만들기·이름 변경·인쇄(`Ctrl/⌘ + P`)·휴지통 이동을, 수정 메뉴는 값·서식·행열 바꿔 붙여넣기를, 보기 메뉴는 눈금선과 전체 화면을, 삽입 메뉴는 함수와 링크를, 서식 메뉴는 글꼴·크기·텍스트 색·채우기 색·회전·테이블 서식·테두리를, 데이터 메뉴는 선택 열 기준 정렬과 중복 항목 삭제·공백 제거·텍스트를 열로 분할을 제공합니다. 도움말 메뉴의 **함수 목록** 은 서버의 `GET /api/v1/formula/functions` 카탈로그를 그대로 보여 줍니다.
