...
export function EditorPage({workbookId,build,session}:{workbookId:string;build?:BuildInfo;session?:Session}) {
  ...
  const menus:WorkbookMenu[]=[
    ...
    {label:'삽입',items:[
      ...
      {kind:'item',label:'표…',onSelect:()=>setSheetTableOpen(true)},
  ...
  return <div className="editor-shell">
    ...
    {sheetTableOpen&&<SheetTableDialog selection={editorSelection} activeSheetId={activeSheet.id} sheets={workbook.data.sheets} tables={sheetTables.data?.items??[]} onClose={()=>setSheetTableOpen(false)} onCreate={createSheetTable} onUpdate={updateSheetTable} onDelete={deleteSheetTable} onNavigate={item=>{navigateToRange(item.sheet_id,item.range);setSheetTableOpen(false)}}/>}
