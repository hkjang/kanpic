...
type Draft={name:string;range:string;ruleType:ConditionalFormatRuleType;operator:ConditionalFormatOperator;formula:string;value:string;value2:string;background:string;color:string;bold:boolean;italic:boolean;minColor:string;midColor:string;maxColor:string;useMidColor:boolean;barColor:string;priority:number;stopIfTrue:boolean}

const types:Array<[ConditionalFormatRuleType,string]>=[['value','값 조건'],['custom_formula','맞춤 수식'],['duplicate','중복·고유 값'],['color_scale','색상 범위'],['data_bar','데이터 막대']]
...
export function ConditionalFormatDialog({range,rules,onClose,onCreate,onUpdate,onDelete}:{
  ...
  const changeType=(ruleType:ConditionalFormatRuleType)=>setDraft(current=>({...current,ruleType,operator:ruleType==='duplicate'?'duplicate':ruleType==='value'?'greater_than':current.operator,stopIfTrue:ruleType==='value'||ruleType==='duplicate'||ruleType==='custom_formula'?current.stopIfTrue:false}))
  const save=async()=>{
    ...
    const input:Record<string,unknown>={name:draft.name.trim(),range:draft.range.toUpperCase(),rule_type:draft.ruleType,priority:draft.priority,stop_if_true:draft.ruleType==='value'||draft.ruleType==='duplicate'||draft.ruleType==='custom_formula'?draft.stopIfTrue:false}
  ...
  return <div className="modal-backdrop"><div className="modal validation-modal conditional-format-modal" role="dialog" ref={dialog as React.RefObject<any>} aria-modal="true" aria-label="조건부 서식"><header><div><Palette/><div><h2>조건부 서식</h2><p>값 비교, 맞춤 수식, 중복, 색상 범위와 데이터 막대를 서버 권위 규칙으로 표시합니다.</p></div></div><button aria-label="조건부 서식 닫기" onClick={onClose}>×</button></header><div className="validation-layout"><aside><button className={!selected?'active':''} onClick={()=>choose()}><Plus/> 새 서식 규칙</button>{rules.map(rule=><button key={rule.id} className={selected?.id===rule.id?'active':''} onClick={()=>choose(rule)}><span>{rule.name||rule.range}</span><em>{rule.range} · {ruleTypeLabel(rule.rule_type)} · 우선순위 {rule.priority}</em></button>)}</aside><section><div className="validation-fields three"><label>규칙 이름<input aria-label="조건부 서식 이름" maxLength={200} value={draft.name} onChange={event=>setDraft(current=>({...current,name:event.target.value}))} placeholder="예: 목표 초과"/></label><label>적용 범위<input aria-label="조건부 서식 범위" value={draft.range} onChange={event=>setDraft(current=>({...current,range:event.target.value}))}/></label><label>규칙 유형<select aria-label="조건부 서식 유형" value={draft.ruleType} onChange={event=>changeType(event.target.value as ConditionalFormatRuleType)}>{types.map(([value,label])=><option value={value} key={value}>{label}</option>)}</select></label></div>
