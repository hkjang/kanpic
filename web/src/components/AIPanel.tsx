...
const modeLabel:Record<AIAction['mode'],string>={formula:'수식 생성',explain:'수식 설명',fix:'수식 오류 수정',summarize:'범위 요약',anomaly:'이상치 탐지',clean:'데이터 정제'}
    ...
    <div className="ai-scroll">
      ...
      {config.data?.enabled&&<>
        ...
        {!action&&<>
          ...
          <div className="suggestion-list"><button onClick={()=>choose('summarize','선택 범위의 핵심 지표와 패턴을 요약해줘')}>핵심 지표 요약</button><button onClick={()=>choose('anomaly','선택 범위에서 평균이나 패턴에서 벗어난 이상치를 찾아줘')}>이상치 탐지</button><button onClick={()=>choose('clean','선택 범위의 공백과 대소문자, 숫자·날짜 형식을 일관되게 정제해줘')}>데이터 형식 정제</button><button onClick={()=>choose('formula','선택 범위의 빈 열에 각 행의 합계를 계산하는 수식을 제안해줘')}>행별 합계 수식</button><button onClick={()=>choose('explain','선택한 수식의 계산 방식과 참조 범위를 설명해줘')}>선택 수식 설명</button><button onClick={()=>choose('fix','선택한 수식의 오류 원인을 찾아 수정안을 제안해줘')}>수식 오류 수정</button></div>
        </>}
        ...
        {!action&&<form className="ai-composer" onSubmit={submit}><label>작업 유형<select aria-label="AI 작업 유형" value={mode} onChange={event=>setMode(event.target.value as AIAction['mode'])}><option value="summarize">범위 요약</option><option value="anomaly">이상치 탐지</option><option value="clean">데이터 정제</option><option value="formula">수식 생성</option><option value="explain">수식 설명</option><option value="fix">수식 오류 수정</option></select></label><textarea aria-label="AI 요청" value={request} onChange={event=>setRequest(event.target.value)} maxLength={4000} placeholder="선택한 범위에 수행할 작업을 입력하세요…"/><button className="primary" disabled={!request.trim()||plan.isPending}>{plan.isPending?<RefreshCw className="spin"/>:<Send/>} {plan.isPending?'계획 생성 중':'분석 및 계획 미리보기'}</button><small><Eye/> 승인 전에는 워크북을 변경하지 않습니다.</small></form>}
