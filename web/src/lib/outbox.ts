...
export async function flushOutbox(onApplied?: (operation:OutboxOperation, result:unknown)=>void) {
  ...
  for (const item of items) {
    try {
      const response = await fetch(`/api/v1/sheets/${item.sheetId}/cells:batch`, { method:'PATCH', credentials:'same-origin', headers:{'Content-Type':'application/json'}, body:JSON.stringify(item.body) })
      if (!response.ok) {
