...
test('collaborates with anchored comments, replies, and mention notifications', async ({ page, context }) => {
  ...
  expect(comments.items[0].range).toBe('B2:C3')

  const reviewer=await context.newPage()
  await reviewer.setExtraHTTPHeaders({'X-Kanpic-Actor':'bob@example.com'})
  await reviewer.goto('/')
  const bell=reviewer.getByRole('button',{name:/알림 1개/})
  await expect(bell).toBeVisible()
  await bell.click()
  const notification=reviewer.locator('.notification-list>button').filter({hasText:'B2:C3'})
  ...
  await expect(reviewer.locator('.name-box')).toHaveValue('B2:C3')
