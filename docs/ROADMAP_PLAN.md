# kanpic 단기·중기·장기 로드맵 및 사업/기술 실행 계획서
...
gantt
    ...
    section 중기 (3~6개월)
    OT/CRDT 실시간 동시편집 락킹  :b1, 2026-11-01, 60d
    Canvas 차트 시각화 모듈       :b2, 2026-11-15, 45d
    사내 ERP/CRM API 커넥터 구축  :b3, 2026-12-01, 45d
    section 장기 (6~12개월+)
    자율형 AI Autonomous Agent    :c1, 2027-02-01, 90d
    HA 분산 클러스터링 배포       :c2, 2027-03-01, 60d
...
단기 목표는 현 v0.3.0 버전의 데이터 유효성 검사 및 OIDC 통합 안정화를 기반으로 **사내 AI 에이전트 실시범 연동과 성능 최적화**를 달성하는 것입니다.

```mermaid
graph LR
    subgraph ShortTerm [단기 실행 과제 1~3개월]
        A[MCP 2.0 AI 에이전트 연동] --> B[대용량 XLSX/CSV I/O 최적화]
        B --> C[Custom Formula 검증 고도화]
        C --> D[재무/기획 부서 PoC 수행]
    end
```
...
1. **Model Context Protocol (MCP) 2.0 연동 고도화**
   - 사내 LLM 기반 AI 에이전트가 `/mcp` 엔드포인트를 통해 워크북 데이터 자동 요약, 이상치 감지, 수식 오류 수정을 직접 수행하도록 도구(Tools) 확장.
...
```mermaid
graph TB
    subgraph CorePlatform [kanpic Enterprise Platform]
        Engine[Go Core Engine]
    end

    subgraph Extensions [중기 고도화 모듈 3~6개월]
        CRDT[OT/CRDT 실시간 동시 편집 락킹]
        Charts[Canvas 2D 차트 & 시각화 엔진]
        Connectors[ERP / MES / CRM 데이터 커넥터]
        RBAC[RBAC 고급 역할 기반 권제어]
    end
...
1. **OT / CRDT 기반 실시간 동시 편집 및 락킹 (Co-editing & Cell Locking)**
   ...
   - 셀 단위 실시간 편집 락킹 표시(사용자 아바타 하이라이트).
...
```mermaid
graph TD
    subgraph LongTermVision [장기 비전: 6~12개월+]
        A[Autonomous AI Agent - 스스로 분석/보고서 작성]
        B[HA 분산 클러스터링 & Multi-Region 배포]
        C[Parquet / DuckDB 올랩 Big Data 분석 엔진]
        D[전사 MS Excel 완전 대체 및 표준화]
    end

    A --> EnterpriseStandard[전사 표준 AI 데이터 협업 플랫폼]
    B --> EnterpriseStandard
    C --> EnterpriseStandard
    D --> EnterpriseStandard
```

### 4.1 주요 핵심 과제

1. **자율형 AI Autonomous Data Agent 구축**
   - 사용자의 자연어 명령("지난 3개년 매출 추이를 분석하고 경영진 보고용 차트와 요약을 작성해줘")을 받아 AI 에이전트가 워크북 생성, 수식 적용, 데이터 유효성 검사, 시각화를 스스로 완성.

2. **초대용량 OLAP 빅데이터 처리 (DuckDB / Parquet 연동)**
   - 백만 행 이상의 대용량 빅데이터를 브라우저 및 서버 단에서 DuckDB/Parquet 기술을 활용하여 초고속 분석.
...
| **중기 (3~6개월)** | 동시 편집 충돌율 / 커넥터 연동 수 | 0.01% 이하 / 사내 주요 3개 시스템 |
