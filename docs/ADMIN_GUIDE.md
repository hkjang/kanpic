...
- **세션 종료**: **모든 세션 종료** 는 해당 사용자의 브라우저 세션을 모두 삭제합니다. 기기 분실이나 권한 변경 직후에 사용합니다.
...
응답 길이는 기본적으로 서버가 정합니다. `GET /v1/models` 응답의 `max_model_len`(vLLM) 또는 `context_length`를 읽어 컨텍스트 길이를 파악하고, 프롬프트 추정 토큰과 여유분을 뺀 값을 그 요청의 `max_tokens`로 사용합니다. 값은 10분간 캐시하며, 컨텍스트 길이를 제공하지 않는 Gateway에서는 보수적인 기본값을 쓰고 응답이 잘리면 상한을 키워 자동으로 다시 요청합니다. `ai.max_output_tokens`를 지정하면 그 값이 항상 우선합니다. `ai.model`에 적은 이름이 목록에 없고 모델이 하나뿐이면 그 모델을 사용합니다.
