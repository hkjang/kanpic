...
- **이메일은 바꿀 수 없습니다.** 알림 메일은 디렉터리에 적힌 이메일로 갑니다 — 지켜보기 알림에는 바뀐 칸이, 멘션에는 댓글이 실려 나갑니다. 부서 관리자가 그것을 바꿀 수 있으면 구성원의 알림을 조용히 자기 쪽으로 돌릴 수 있고, 받는 사람은 메일이 끊긴 것을 한참 뒤에야 압니다. 이름과 메모는 그대로 고칠 수 있습니다.
- **관리자 계정은 다룰 수 없습니다.** 맡은 부서 구성원 중에 전역 관리자가 섞여 있어도 정지·세션 종료·조회가 되지 않고, **사용자 목록에도 나오지 않습니다.** 부서 하나를 맡겼을 뿐인데 관리자를 콘솔에서 잠가 조직의 관리 자체를 멈출 수 있으면 위임이 아닙니다.
- **관리자 역할은 건드릴 수 없습니다.** 부서 관리자가 구성원에게 `kanpic-admin` 을 얹으면 전역 관리자를 하나 만들어 내는 셈이고, 자기 자신을 막아 둔 것도 그 한 걸음으로 우회됩니다. 회수도 맡은 범위 밖이므로 함께 막습니다. 그 밖의 kanpic 역할은 그대로 부여·회수할 수 있습니다.
...
- 받은 워크북에는 어디서 왔는지가 남습니다. 편집기 제목 아래에 **ptium 에서 받음** 처럼 허용 목록의 이름으로 보이고(마우스를 올리면 오리진·파일 이름·받은 시각), `GET /api/v1/workbooks/{workbookId}/handoff` 가 같은 것을 오리진·파일 이름·받은 사람·시각과 허용 목록의 이름(`service`)으로 답합니다. 그 서비스를 허용 목록에서 지우면 이름 대신 호스트로 보입니다.
...
`/admin` → **워크북 거버넌스** 에서 **링크 공개** 나 **조직 전체** 를 고르면 목록 위에 **이 목록 전체 공개 해제** 가 나옵니다.

`링크 공개 47개` 를 보고 마흔일곱 번 누르는 사람은 없습니다. 몇 개는 남고, **남은 것은 아무도 세지 않습니다.**

- 고른 거르개만 닫습니다. `링크 공개` 를 눌렀다고 `조직 전체` 가 함께 닫히지 않습니다.
- 한 번에 **500개까지** 닫고 **몇 개가 남았는지 세어** 알려 줍니다. 조직이 크면 요청 하나가 몇 분씩 걸리는 편보다, 몇 번 나눠 누르되 남은 수를 아는 편이 낫습니다.
- 못 닫은 것이 있으면 그 수도 함께 적습니다.
- 일괄 해제한 일은 **관리자 행위 기록** 에 남습니다.

### API 키 폐기

`/admin` → **API 키 현황** 에서 키마다 **폐기** 를 누르면 그 키만 끊습니다.

키 하나가 새어 나갔을 때 지금까지는 그 사람 계정을 통째로 정지하는 수밖에 없었습니다. 그러면 **키와 상관없는 그 사람의 일까지 멈춥니다.** 새어 나간 것만 끊는 편이 맞습니다.

- 폐기는 되돌릴 수 없습니다. 그 키를 쓰던 연동은 즉시 끊깁니다.
- 폐기한 일은 **관리자 행위 기록** 에 남습니다.
- 참고: 계정을 정지하면 그 사람의 API 키도 함께 막힙니다. 사람이 떠날 때는 정지로 충분하고, 폐기는 **사람은 남고 키만 문제일 때** 씁니다.

## 7.3 관리자 행위 기록 (감사 추적)

관리자 콘솔에서 **사람에게 영향을 준 일** 은 서버 로그에 남습니다. 계정 등록·정지·수정, 역할 부여·회수, 세션 종료, 사용자 일괄 등록, 워크북 소유권 이전(하나씩·일괄), 부서 관리자 지정·해제입니다.

`/admin` → **서버 로그** 에서 **관리자 행위만** 을 켜면 그것만 모아 봅니다. 각 줄에는 이렇게 적힙니다.
...
- `actor` 는 **누가** 했는지입니다.
- `scope` 는 **전역 관리자였는지 위임받은 부서 관리자였는지** 입니다. 감사에서 먼저 묻는 것이 "이 사람이 그럴 권한이 있었는가" 입니다.
- 기록은 서버 로그와 같은 곳에 쌓이므로 기간으로 거르고 **CSV로 내보내는 것이 그대로 됩니다.**
