...
**일정표** 는 열을 앞에서부터 **이름 · 시작 · 끝** 으로 읽어 가로 막대로 그립니다. 끝을 적지 않으면 그 날 하루짜리 이정표로 봅니다. 끝이 시작보다 앞이면 거꾸로 적은 것으로 보고 바로잡아 그립니다 — 그대로 두면 막대가 뒤로 자라 읽을 수 없습니다. 날짜는 `=DATE(2026,1,5)` 같은 글자도, 파일에서 들어온 날 수도 함께 읽습니다. 일감이 많아 한 줄이 6픽셀보다 얇아지면 그릴 수 있는 만큼만 그리고 **몇 개가 남았는지 적습니다** — 그림 밖으로 넘쳐 흐르는 것보다 낫습니다. 일정표는 엑셀에 같은 것이 없어 XLSX 로 내보낼 때는 빠집니다.
