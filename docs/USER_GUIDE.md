...
- **함께 보는 문서**: 설치·설정·운영은 [관리자 가이드](ADMIN_GUIDE.md)에 있습니다.
...
### 1.3 처음 5분 — 로그인부터 첫 표와 차트까지

처음 쓰는 사람이 한 번에 따라 할 수 있는 길입니다. 다섯 단계면 데이터가 담긴 워크북과
그 데이터를 그린 차트까지 만들어집니다.

**1단계. 로그인합니다.** 브라우저에서 관리자가 알려 준 주소를 엽니다. 아이디와 비밀번호를
넣고 **관리자로 로그인** 또는 **조직 계정으로 로그인** 을 누릅니다.

![로그인 화면 — 아이디·비밀번호로 들어가거나 조직 계정(Keycloak)으로 들어간다](images/guide/login.png)

**2단계. 워크북을 만듭니다.** 홈 화면 오른쪽 위의 **+ 새 워크북** 을 누르면 빈 워크북이
바로 열립니다. 이미 있는 파일로 시작하려면 **파일 가져오기** 를 누릅니다.

![홈 화면 — 빠른 시작 템플릿과 최근 워크북 목록](images/guide/home.png)

**3단계. 값을 넣습니다.** 셀을 고르고 그대로 입력합니다. `A1` 부터 표의 머리글과 값을
채운 뒤, 합계 칸에 `=SUM(B2:D2)` 처럼 적으면 바로 계산됩니다. 저장 단추는 없습니다 —
입력은 서버에 자동으로 반영되고, 워크북 이름 아래에 **모든 변경사항 저장됨** 과 현재
버전이 표시됩니다.

![편집기 — 값과 수식을 넣은 표. 왼쪽 위 이름 상자와 수식 입력줄, 아래 상태 표시줄의 합계·평균·개수](images/guide/editor.png)

**4단계. 차트로 봅니다.** 표 범위를 끌어 고른 뒤 도구 모음 오른쪽의 **차트 패널** 을 열고
**새 차트** 를 누릅니다. 제목을 적고 **차트 저장** 을 누르면 시트 위에 차트가 놓입니다.

![차트 패널 — 고른 범위로 만든 막대 차트가 시트 위에 놓인다](images/guide/editor-chart.png)

**5단계. 함께 봅니다.** 오른쪽 위 **공유** 에서 사람·부서·역할을 더하고 권한을 정합니다.
일이 끝난 지점에는 **버전 이력** 에서 이름을 붙여 두면 나중에 그 시점으로 되돌릴 수 있습니다.

여기까지 했으면 나머지는 필요할 때 찾아 읽으면 됩니다. 화면별 사용법은 3장(홈)과 4장(편집기),
자주 하는 작업은 8장(공유)·10장(차트)·11장(피벗)·18장(프레젠테이션),
막혔을 때는 20장을 보세요.
...
2. 로그인 화면에서 사용자 ID와 비밀번호를 입력 후 **[관리자로 로그인]** 버튼을 클릭합니다.

로그인 화면 왼쪽 아래에는 지금 접속한 서버의 버전과 빌드가 적혀 있습니다. 문의할 때
이 값을 함께 알려 주면 관리자가 확인하기 쉽습니다.

### 2.2 Keycloak OIDC SSO (Single Sign-On) 로그인
사내 SSO 인증 체계(Keycloak)와 연동된 경우 로그인 화면의 **회사 계정으로 SSO 로그인**을 누르면 원클릭 인증으로 접속할 수 있습니다. bootstrap 관리자 로그인은 그 아래 **관리자 계정으로 로그인**을 눌러야 펼쳐지는 복구용 통로입니다.

관리자가 자동 로그인(Silent SSO)을 켠 설치에서는 회사 계정에 이미 로그인돼 있으면 이 화면을 거치지 않고 바로 들어갑니다. 회사 계정 세션이 없으면 "회사 계정 세션이 없어 자동으로 로그인하지 않았습니다" 라는 안내와 함께 로그인 화면이 뜨니, 그때는 **회사 계정으로 SSO 로그인**을 누르면 됩니다. 스스로 로그아웃한 뒤에는 다시 로그인할 때까지 자동으로 들어가지 않습니다.
...
![홈 화면 — 위쪽에 검색과 빠른 시작 템플릿, 아래에 최근·내 소유·나와 공유됨·즐겨찾기·휴지통으로 거를 수 있는 워크북 목록](images/guide/home.png)

화면은 세 부분입니다.

- **맨 위 줄**: 워크북·시트·데이터를 찾는 검색창(`Ctrl/⌘+K`), 알림, 프로필 메뉴.
  프로필 메뉴에서 개인 환경설정과 (권한이 있으면) 관리자 콘솔로 갑니다.
- **빠른 시작**: 빈 워크북과 추천 템플릿. 오른쪽 **템플릿 갤러리** 로 전체 목록을 엽니다.
- **최근 워크북**: 카드 하나가 워크북 하나입니다. 이름으로 찾을 수 있고,
  **최근 / 내 소유 / 나와 공유됨 / 즐겨찾기 / 휴지통** 으로 목록을 바꿉니다.
  카드의 `...` 메뉴에는 이름 변경·복사·공유·링크 복사·휴지통이 들어 있습니다.
...
- **파일 가져오기 (Import)**:
  ...
  - 미리보기 대화상자에서 읽어 들인 행·열과 경고를 확인하고 **[워크북으로 가져오기]**를 실행합니다.

  ![파일 가져오기 미리보기 — 고른 CSV 의 시트 수, 행·열과 비어 있지 않은 셀 수를 먼저 보여 준다](images/guide/home-import.png)

  - 인코딩은 파일이 스스로 밝힌 대로 읽습니다. UTF-8 은 물론, 파일 앞에 표시(BOM)를 두는 UTF-16LE·UTF-16BE — 윈도우 파워셸의 `Export-Csv` 와 엑셀의 `유니코드 텍스트` 저장이 그렇게 씁니다 — 도 그대로 올릴 수 있습니다. 표시가 없는 EUC-KR 같은 파일은 무엇인지 알 길이 없어 거절하므로, 저장할 때 UTF-8 을 고르세요.
...
![편집기 화면 — 위에서부터 워크북 이름과 저장 상태, 메뉴 막대, 도구 모음, 이름 상자와 수식 입력줄, 격자, 시트 탭과 상태 표시줄](images/guide/editor.png)

위에서부터 이렇게 놓여 있습니다.

- **제목 줄**: 워크북 이름과 저장 상태(**모든 변경사항 저장됨 · v2**), 지금 함께 보고 있는
  사람 수, **XLSX 내보내기 · CSV**, **공유**.
- **메뉴 막대**: 파일 · 수정 · 보기 · 삽입 · 서식 · 데이터 · 도구 · 도움말.
- **도구 모음**: 실행 취소·다시 실행, 워크북 검색, 글꼴과 크기, 굵게·기울임·밑줄, 색,
  정렬·병합·서식 복사·테두리, 정렬·필터·데이터 검증·조건부 서식, 확대·축소, 그리고 오른쪽 끝의
  패널 단추들(차트·피벗·자동화·AI 도우미·편집 충돌·버전 이력·댓글).
- **이름 상자와 수식 입력줄**: 왼쪽 상자에 `C7` 이나 `A1:D9` 를 적으면 그 범위로 갑니다.
  오른쪽 `fx` 줄에는 지금 셀에 저장된 값 또는 수식이 그대로 보입니다.
- **상태 표시줄**: 고른 범위의 합계·평균·개수와 저장 상태.
...
**여러 시트 합치기(통합)** 는 여러 시트의 같은 모양 표를 이름표 기준으로 합칩니다. 1월·2월·3월 시트가 부서 × 항목으로 되어 있으면 부서와 항목을 맞춰 더합니다 — **시트마다 부서 차례가 달라도 됩니다.** 자리로 맞추면 1월 영업1팀에 2월 영업2팀이 더해집니다. 합계·개수·평균·최대·최소 중에서 고릅니다.

첫 행은 항목 이름, 첫 열은 줄 이름표로 봅니다. 어느 시트에만 있는 이름표도 결과에 들어가고, **없는 칸은 비워 둡니다** — 없는 것은 0이 아니라 세지 않은 것이고, 0을 적으면 실적이 0원이었다는 뜻이 됩니다. 무엇이 숫자인지는 `=SUM` 과 같은 규칙으로 보므로 `"1,234"` 처럼 글자로 담긴 숫자는 빠지고, **몇 칸이 빠졌는지 세어 알려 줍니다.** 병합된 셀이 있는 시트도 이름을 짚어 줍니다 — 병합된 이름표 열은 빈 이름표 행을 만들어 그 행이 통째로 빠지기 때문입니다. 표가 A1에서 시작하지 않아도 됩니다 — 시트마다 값이 있는 자리를 찾아 그곳을 표로 봅니다. 한 번에 시트 열두 개까지 읽고, 그보다 많으면 몇 개만 읽었는지 말합니다. 결과는 새 시트에만 적습니다.

**두 목록 비교(대사)** 는 두 범위를 키 열로 견주어 어느 쪽에만 있는 항목인지 찾습니다. 비교할 범위는 `장부!A1:C100` 처럼 시트 이름을 붙여 적습니다 — 대사는 대개 시트 두 개를 견주는 일이기 때문입니다. **한쪽이 `"1,000"` 글자이고 다른 쪽이 `1000` 숫자여도 같은 항목으로 맞춥니다.** 은행에서 받은 내역과 장부는 거의 항상 이렇게 다르고, 글자 그대로 견주면 하나도 맞지 않습니다. **번호는 번호로 둡니다** — `007` 은 `7` 과 다른 항목이고, 열다섯 자리를 넘는 계좌번호는 수로 읽지 않습니다. 수로 읽으면 뒤가 뭉개져 서로 다른 계좌번호 둘이 한 항목이 됩니다. 대사에서 틀리게 "맞았다" 고 하는 것은 "안 맞았다" 고 하는 것보다 나쁩니다 — 안 맞으면 사람이 들여다보지만, 맞았다고 하면 거기서 끝납니다. 같은 키가 한쪽에서 거듭 나오면 그것도 함께 알려 줍니다 — 대사에서는 그 자체가 발견입니다. 키 칸이 비어 견주지 못한 행의 수도 적습니다. 조용히 빼면 "다 맞았다" 는 대사표가 나오고 수는 맞지 않습니다. 결과는 **새 시트에만** 적고 원래 자료는 건드리지 않습니다. **행이 움직이는 정리(중복 항목 삭제·부분합 제거)와 텍스트를 열로 분할은 병합된 셀을 만나면 멈추고 어느 칸이 걸렸는지 알려 줍니다.** 정렬과 같은 규칙입니다 — 병합을 먼저 해제한 뒤에 하세요. 공백 제거와 텍스트로 저장된 숫자는 제자리에서 고치므로 병합과 상관없이 됩니다.
...
| 데이터 | 정렬(선택 열 기준 A→Z·Z→A, 범위 정렬), 두 목록 비교, 여러 시트 합치기, 필터 보기, 슬라이서 추가, 데이터 연결, 데이터 검증, 피벗 테이블, 이름 범위, 데이터 정리(표 검사·중복 항목 삭제·공백 제거·텍스트로 저장된 숫자·병합 해제하고 채우기·부분합 제거, 미리보기), 텍스트를 열로 분할(미리보기), 부분합, 행·열 숨기기, 워크북 검색 |
...
날짜 조건은 `2023-03-15`, `2023-03-15 16:12:00`, `2023-03-15T16:12:00Z` 세 가지 꼴만 날짜로 읽습니다. `2023/03/15` 나 `March 15, 2023` 은 브라우저가 읽어 줄 수 있지만 **서버가 읽지 않으므로 화면도 읽지 않습니다.** 화면만 너그러우면 "괜찮다" 하고 적어 넣은 값을 서버가 물리쳐, 값이 나타났다 사라지는 것을 보게 됩니다. 자리를 적지 않은 꼴은 UTC로 봅니다.
...
HTML이 없는 곳(메모장, 터미널)에서 붙여넣을 때도 보이던 글자를 숫자로 읽습니다. `1,234`, `98,000원`, `(1,234)` 는 숫자로 들어오고 보이던 모습은 표시 형식으로 남습니다. 쉼표가 세 자리씩 끊지 않는 `1,2` 처럼 자릿수 구분으로 볼 수 없는 값, 전화번호, 날짜는 글자 그대로 둡니다. `007` 처럼 앞자리 0이 뜻을 갖는 우편번호·사번과 열다섯 자리를 넘는 계좌·전표 번호도 글자로 둡니다 — 숫자로 바꾸면 앞의 0이 사라지거나 뒷자리가 뭉개져 되돌릴 수 없기 때문입니다. 같은 표를 CSV·TSV로 가져올 때와 같은 규칙입니다. XLSX는 앞자리 0만 같습니다 — 엑셀이 수로 저장해 둔 칸은 이미 실수여서 자릿수를 되돌릴 것이 없기 때문이며, 그래서 다른 도구가 형식 없이 적어 둔 아주 긴 번호는 XLSX로 가져올 때 수로 들어올 수 있습니다. 긴 번호가 걱정되면 CSV로 올리세요. (셀에 직접 쳐 넣은 `007` 은 스프레드시트가 늘 하던 대로 숫자 7이 됩니다. 밖에서 옮겨 온 표만 적힌 대로 지킵니다.)
...
**고르는 자리에서 몇 장으로 나뉘는지 바로 보여 줍니다.** 방향이나 여백을 바꾸면 그 자리에서 장수가 바뀌고, 어느 열이 어느 장에 가는지도 함께 적힙니다 — 종이가 나온 뒤에 아는 것은 늦습니다. 이 셈은 인쇄가 실제로 쓰는 것과 **같은 코드이고, 같은 칸을 읽어 같은 범위** 를 씁니다. 따로 세면 "한 장에 들어갑니다" 라고 보여 주고 두 장을 뱉는 일이 생기고, 그러면 미리보기는 없느니만 못합니다. 세로 길이는 종이 크기와 행 높이에 따라 브라우저가 정하므로 장수를 적지 않습니다 — 모르는 것을 아는 척하지 않습니다.
...
- 숫자가 아닌 셀은 **개수** 에만 반영되고 합계·평균에는 들어가지 않습니다. 무엇이 숫자인지는 **수식 엔진과 같은 규칙** 을 씁니다 — `"2000"` 은 세고 `"1,234"` 는 세지 않습니다. 화면에 보이는 합계와 `=SUM` 이 내는 값이 다르면 어느 쪽을 믿어야 할지 알 수 없기 때문입니다.
- **표 검사** 는 표를 훑어 고칠 만한 것을 **세어 보여 줍니다.** 글자로 담긴 숫자 몇 칸, 병합 몇 곳, 부분합 몇 행, 중복 몇 행, 공백 몇 칸인지 한자리에서 알려 주고, 각 줄에서 바로 그 정리의 미리보기로 넘어갑니다. 정리 도구를 하나씩 열어 보지 않아도 됩니다 — 무엇이 잘못됐는지 모르는 채로 열어 보는 사람은 없기 때문입니다. **검사는 아무것도 바꾸지 않습니다.** 무엇을 고칠지는 각 갈래의 미리보기를 보고 정합니다. △ 표시가 붙은 것은 고치면 **셈이 달라지는** 것이고, 나머지는 모양만 다듬습니다.
- **병합 해제하고 채우기** 는 병합을 풀고 그 값을 가려져 있던 칸에 채웁니다. 부서 이름이 세 행에 걸쳐 한 번만 적힌 표는 사람 눈에는 읽기 좋지만 **정렬도 피벗도 되지 않습니다** — 둘째·셋째 행에는 부서가 적혀 있지 않기 때문입니다. 가려져 있던 칸에 이미 값이 있으면 그 값을 남깁니다. kanpic의 병합은 값을 지우지 않고 가리기만 하므로, 덮어 채우면 보지 못하던 값을 없애는 일이 됩니다. 병합이 고른 범위 밖까지 뻗어 있으면 밖까지 함께 풉니다.
- 쉼표나 통화 기호가 붙어 **글자로 담긴 숫자** 는 **데이터 › 데이터 정리 › 텍스트로 저장된 숫자…** 로 고칩니다. `1,234` `₩5,000` `(500)` `50%` 를 알아보고 바꿔 줍니다. 괄호는 음수로, 백분율은 100으로 나눕니다. 열다섯 자리를 넘는 숫자는 바꾸지 않습니다 — 그런 것은 금액이 아니라 계좌번호이고, 바꾸면 뒤가 뭉개집니다. `007` 처럼 앞자리 0이 붙은 정수도 바꾸지 않습니다 — 우편번호·사번이라 0이 사라지면 되돌릴 수 없습니다. 정말 수로 만들려면 그 칸에 직접 쳐 넣으세요. 통화 기호와 뒤에 붙은 단위(`원`·`USD`)도 표시 형식으로 옮겨 그대로 보입니다. CSV로 가져올 때는 쉼표나 통화 기호가 붙은 값이 글자로 들어옵니다. 가져오기 미리보기가 **몇 칸인지 세어 알려 주므로**, 가져온 뒤 이 정리로 한 번에 고치면 됩니다. 열다섯 자리를 넘는 수는 CSV에서도 글자로 둡니다 — 실수로 읽으면 파일에 적힌 것과 다른 값이 칸에 들어가고 되돌릴 수 없습니다.

붙여넣기로 들어오는 값도 **같은 규칙** 으로 읽습니다 — 같은 `₩5,000` 이 어떻게 들어왔는지에 따라 다르게 저장되지 않습니다. **보이던 모양은 그대로 둡니다** — `₩` 와 `%` 는 표시 형식으로 옮기므로, 칸에 담기는 값만 숫자로 바뀌고 화면은 달라지지 않습니다. 쉼표가 세 자리마다 찍혀 있지 않으면 숫자로 보지 않습니다 — 억지로 바꾸면 사람이 적은 것과 다른 값이 들어갑니다.
...
**가정 한 벌에 이름을 붙여 두고 나란히 놓고 견줍니다.** **데이터 › 가정 분석 › 시나리오…** 에서 만듭니다.
...
가정은 한 줄에 하나씩 `셀=값` 으로 적습니다. 데이터 표가 한두 칸을 여러 값으로 바꿔 보는 것이라면, 시나리오는 **여러 칸을 한 벌로 묶는** 것입니다 — 회의에서 두 안을 놓고 보는 그 일입니다.

**지금 값을 함께 보여 줍니다.** 무엇에서 무엇으로 바뀌는지 알아야 견주는 뜻이 있습니다.

**시트는 그대로 둡니다.** 벌마다 원래 값으로 되돌린 뒤 그 벌의 가정만 넣으므로, 앞 벌의 가정이 뒤 벌에 섞이지 않습니다.

행과 열이 움직이면 가정이 가리키는 칸도 함께 움직입니다. 가리키던 칸이 지워지면 **그 가정만** 빠집니다 — 한 칸이 사라졌다고 시나리오를 통째로 버리면 적어 둔 것을 다 잃습니다.

#### 데이터 표

**가정을 하나씩 넣어 보며 결과가 어떻게 달라지는지 한 번에 봅니다.** **데이터 › 가정 분석 › 데이터 표…** 에서 결과 셀과 바꿀 셀, 그리고 가정들을 적습니다.
...
가로 쪽을 비우면 한 방향 표가 됩니다. 목표값 찾기와 **반대 방향의 물음** 입니다 — "얼마여야 이 값이 나오나" 가 아니라 "이 값이 이만큼일 때 결과가 어떻게 되나" 입니다.

**시트는 그대로 둡니다.** 가정을 넣어 본 것이 남으면 표를 한 번 그렸다는 이유로 자료가 바뀝니다.

표 한 칸마다 워크북을 통째로 다시 셈합니다. 그래서 **워크북이 크면 표를 작게** 잡아야 합니다 — 너무 크면 셈하기 전에 알려 드립니다. 2,000개 수식이 있는 시트에서 400칸 표가 3초쯤 걸립니다.

셈하지 못한 자리는 **비워 두고 까닭을 적습니다.** 0 으로 채우면 사람은 그것을 답으로 읽습니다. 넣은 값이 결과 셀에 닿지 않으면 그것도 말해 줍니다 — 같은 값으로 가득 찬 표를 내면 그 가정이 결과를 바꾸지 않는다고 읽히기 때문입니다.
...
**표 바로 아래에 `=SUM(매출표[금액])` 을 적어도 됩니다.** 표를 요약하는 칸은 표의 자료가 아니므로 표가 그 줄을 삼키지 않습니다. 삼키면 그 칸이 제 자신을 더해 `#CIRC!` 가 되는데, 적은 사람은 무엇을 잘못했는지 알 길이 없습니다. 나중에 그 줄 다른 칸에 자료를 적어도 마찬가지입니다. 합계 수식을 지우고 보통 값을 적으면 그때부터는 자료 줄이 됩니다.

**마지막 줄을 합계 줄로 삼을 수 있습니다.** 합계 줄은 자료에서 빠지므로, 그 줄에 `=SUM(매출표[금액])` 을 적어도 **제 자신을 더하지 않습니다.** 빼지 않으면 저장할 때마다 값이 불어나는 순환이 됩니다. `매출표[#합계]` 로 그 줄만 가리킬 수도 있습니다.

합계 줄이 있는 표는 아래 줄을 삼키지 않습니다 — 그 줄이 합계 줄이기 때문입니다. 자료를 더하려면 합계 줄 위에 행을 끼웁니다.

**XLSX 로 내보낼 때는 합계 줄을 표 범위에서 뺍니다.** 파일 안의 표에도 합계 줄이라는 것이 있지만 지금 쓰는 라이브러리가 그것을 적어 주지 못합니다. 넣은 채로 내보내면 도로 열었을 때 그 줄이 자료가 되어 합계 칸이 제 자신을 더합니다. 빼서 내보내면 합계 칸은 표 바로 아래의 보통 칸이 되고, 엑셀에서도 그대로 셈합니다.

**표 이름은 자동완성에 나오고, 이름 상자로 찾아갈 수 있습니다.** 수식에서 `=SUM(매` 까지 치면 `매출표[열 이름]` 을 권하고, 이름 상자에 `매출표` 를 적으면 그 표로 갑니다. 이름 범위와 같습니다 — 한쪽만 되면 사람은 왜 되고 안 되는지 알 수 없습니다.

**표는 격자 위에 테두리와 이름으로 보이고, 색을 고를 수 있습니다.** 머리글 줄에는 옅은 색이 깔리고, 표 이름이 왼쪽 위에 붙습니다. 파랑·초록·주황·빨강 가운데 고른 색으로 그리며, **그 색이 XLSX 로 내보낼 때 엑셀의 표 서식이 되고, 가져올 때 그대로 돌아옵니다** — 화면과 파일이 다른 색이면 어느 쪽이 맞는지 알 수 없기 때문입니다. 엑셀에서 만든 표가 kanpic 이 모르는 서식을 쓰고 있으면 기본색으로 그립니다. 표가 여럿일 때 어디까지가 어느 표인지 한눈에 갈립니다. 칸에 색을 칠해 두는 것이 아니라 그려 주는 것이므로, 표가 행을 따라 움직이면 표시도 함께 움직입니다.
...
- `=REPT(텍스트, 횟수)` / `=HYPERLINK(주소, [표시할 텍스트])`
- `=VALUE(텍스트)`: 글자로 담긴 수를 숫자로 바꿉니다. 가져온 표의 금액 칸처럼 `"1,000"`, `"$1,000"`, `"(1,000)"`(회계 서식의 괄호는 음수), `"12%"` 도 읽습니다. `"12:00"` 은 `=TIMEVALUE` 와 같은 분수(`0.5`), `"2026-01-05"` 는 날 수가 됩니다.
- `=NUMBERVALUE(텍스트, [소수 구분 기호], [자릿수 구분 기호])`: 구분 기호를 직접 정해 읽습니다. 나라마다 표기가 달라 `=NUMBERVALUE("1.234,5", ",", ".")` 은 `1234.5` 입니다. 가운데 빈칸은 무시하고(`"2 500"` → `2500`), 뒤에 붙은 `%` 하나마다 100 으로 나눕니다. 소수 구분 기호가 두 번 나오거나 자릿수 구분 기호가 그 뒤에 오면 `#VALUE!` 입니다.
- `=TEXT(값, 서식)`: 숫자나 날짜를 `#,##0`, `0.0%`, `yyyy-mm-dd` 같은 서식의 글자로 바꿉니다. 서식은 `;` 로 구역을 나눠 **양수 ; 음수 ; 0 ; 글자** 를 따로 적을 수 있습니다. 넷째 구역은 글자 값에만 걸리므로 `#,##0;(#,##0);"-";"["@"]"` 는 숫자 칸은 그대로 두고 `미정` 이라고 적힌 칸만 `[미정]` 으로 그립니다(`@` 가 값이 들어갈 자리입니다). 구역이 셋 이하면 글자는 손대지 않고, 넷째 구역을 비워 두면(`#,##0;;;`) 글자를 감춥니다. 구역이 하나뿐인데 `@` 가 있으면 그 구역이 곧 글자 구역이라 `@" 님"` 은 `홍길동` 을 `홍길동 님` 으로 적습니다. `#,##0;(#,##0);"-"` 은 0 을 `-` 로 적고, 구역을 비워 두면(`0;;`) 그 값은 아무것도 적지 않습니다. 자리 기호 **뒤** 에 붙인 쉼표는 자릿점이 아니라 천 단위로 줄여 적으라는 뜻이라 `#,##0,,` 은 백만 단위(`1234567` → `1`), `0.0,,"백만"` 은 `1.2백만` 입니다. 빗금 양쪽에 자리 기호를 둔 `# ?/?` 는 값을 분수로 적습니다 — `2.75` 는 `2 3/4`, `# ??/??` 는 분모를 99까지 늘려 `0.3125` 를 `5/16` 으로 적고, `?/8` 처럼 분모를 못 박으면 약분하지 않고 `4/8` 로 적습니다. 칸의 표시 형식도 같은 규칙으로 그립니다.
...
- `=FILTER`, `=SORT`, `=UNIQUE`, `=SEQUENCE`, `=TRANSPOSE`, `=FLATTEN`, `=TOCOL`, `=TOROW`, `=ARRAY_CONSTRAIN`, `=GROUPBY`, `=PIVOTBY`: 결과가 여러 셀인 배열 수식입니다. 결과가 펼쳐질 자리에 다른 값이 있으면 `#SPILL!`이 표시되므로 아래쪽을 비워 두십시오.
...
- `=ISBIZNO(값)` / `=ISCORPNO(값)`: 사업자등록번호·법인등록번호의 **검사 숫자** 가 맞는지 봅니다. 하이픈은 있어도 없어도 됩니다. 거래처 명단을 한 번에 훑을 때 씁니다.
- `=FORMATBIZNO(값)`: `2208162517` 을 `220-81-62517` 로 적습니다.
- `=MASKRRN(값, [남길 자리])`: 주민등록번호 뒷자리를 가립니다. `900101-1234567` → `900101-1******`.
- `=HANGULNUM(숫자)`: 수를 한글로 적습니다. `3200000` → `삼백이십만`.
- `=HANGULWON(숫자)`: 금액을 문서에 적는 꼴로 바꿉니다. `3200000` → `일금 삼백이십만원정`. 품의서·세금계산서에 그대로 씁니다.

십·백·천 앞의 `일` 은 뺍니다(`15` → `십오`). 만·억·조 앞에서는 남깁니다(`10000` → `일만`). 문서에 적는 꼴이 그렇습니다.

**원 단위 아래는 오류를 냅니다.** 조용히 버리면 적힌 금액과 셈한 금액이 달라지므로, 반올림은 사람이 뜻을 가지고 하게 둡니다. 9,007,199,254,740,992 를 넘는 금액도 오류입니다 — 표 프로그램의 숫자가 그 너머를 정확히 담지 못해, 적으면 틀린 금액이 조용히 문서에 들어갑니다.

**주민등록번호는 가리기만 하고 검증하지 않습니다.** 2020년 10월부터 새로 매기는 번호는 뒷자리를 임의로 만들어, 검사 숫자가 맞지 않아도 진짜 번호입니다. 검증 함수를 두면 멀쩡한 번호를 틀렸다고 말하게 되는데 그것은 없느니만 못합니다.

`=MASKRRN` 은 열세 자리가 아니면 **오류를 냅니다.** 가릴 수 없는 것을 그대로 돌려주면 가린 줄 알았던 자리에 번호가 그대로 남기 때문입니다.

이 넷은 엑셀에도 구글 시트에도 없어 XLSX 로 내보내면 `#NAME?` 이 됩니다.
...
##### GROUPBY · PIVOTBY — 피벗 없이 집계하기

피벗 테이블을 만들지 않고 수식 하나로 집계합니다. 원본이 바뀌면 결과도 따라 바뀝니다.
...
셋째(GROUPBY)와 넷째(PIVOTBY) 인수는 **함수 자체** 입니다. `SUM` 처럼 이름만 적어도 되고 `LAMBDA(v, SUM(v))` 로 적어도 됩니다.

이어지는 선택 인수는 `[머리글]`, `[총계]`, `[정렬]` 입니다. 머리글은 범위의 첫 줄이 이름 줄인지(기본 아니오), 총계는 `0` 이면 총합계 줄을 붙이지 않고(기본 붙임), 정렬은 `1` 오름차순·`-1` 내림차순·`0` 은 표에 적힌 차례 그대로입니다. 이름표가 여러 칸이면 **앞 칸부터 차례로** 견줍니다 — 부서로 묶고 분기로 다시 묶었으면 분기도 차례가 있어야 합니다. 숫자 이름표는 수로 세우므로 `10` 이 `2` 보다 뒤에 옵니다.

**열 이름표가 여러 칸이면 머리글도 그 칸 수만큼 나옵니다.** 연도 × 분기로 묶으면 윗줄에 연도, 아랫줄에 분기가 적힙니다 — 첫 칸만 적으면 `2026` 이 두 번 나와 어느 쪽이 1분기인지 알 수 없습니다.

**교차점에 자료가 없으면 비워 둡니다.** `0` 을 적으면 "그만큼 팔았다" 는 뜻이 되어 없는 것과 구별되지 않습니다.

엑셀이 받는 인수를 다 받지는 않습니다(`filter_array` 등). 받지 않는 자리에 값을 적으면 **조용히 다르게 셈하지 않고** 무엇까지 되는지 알려 줍니다 — 엑셀에서 옮겨 온 수식이 말없이 다른 답을 내는 것이 가장 나쁘기 때문입니다.

**함수 이름만 적어 넘겨도 됩니다.** `LAMBDA(v, ABS(v))` 를 `ABS` 로 줄여 쓸 수 있습니다 — 엑셀도 같습니다.

```
=MAP(A1:A100, ABS)        =MAP(A1:A100, LAMBDA(v, ABS(v))) 과 같습니다
=BYROW(A1:C10, SUM)       행마다 합계
```

이름 있는 수식도 같은 자리에 넘길 수 있습니다. 다만 같은 이름의 **이름 범위** 를 만들어 두었다면 그쪽이 먼저입니다 — 사람이 그 이름을 만들었다면 그것을 뜻했을 것이기 때문입니다. 칸에 `=SUM` 처럼 함수 이름만 적는 것은 예전과 같이 `#NAME?` 입니다. 함수를 값으로 넘기는 것은 이런 자리에서만 뜻이 있습니다.
...
- CSV·XLSX 가져오기는 한 번에 최대 **1,000,000셀**(업로드 20MB)까지 받습니다. 15만 행 × 3열짜리 CSV(3.5MB)는 2.5초에 들어옵니다. 엑셀 파일의 숫자는 숫자로 들어옵니다. 통화·천 단위 서식이 붙어 있어도 마찬가지이며, `007` 처럼 앞자리 0이 뜻을 갖는 값은 글자로 남습니다. pandas·R 이 내보낸 파일의 `NaN`·`Inf` 같은 값도 글자로 남습니다 — 스프레드시트가 수라고 부르는 것이 아니기 때문입니다. 그런 칸이 있어도 파일은 그대로 들어옵니다.
...
### 6.2 버전 이력 및 원클릭 복원
- 도구 모음 오른쪽의 **버전 이력** 단추를 눌러 패널을 엽니다.
- 맨 위에 현재 워크북 버전이 있고, **이름 있는 버전** 칸에 `3분기 실적 확정` 처럼 적고
  **저장** 하면 그 시점에 이름표가 붙습니다.
- 목록의 항목에는 버전 번호·저장한 사람·시각이 함께 적히며, **복원** 을 누르면 그 시점으로
  돌아갑니다. 복원은 기존 버전을 덮어쓰지 않고 새로운 최신 버전으로 기록되므로,
  되돌린 것을 다시 되돌릴 수 있습니다.

![버전 이력 패널 — 이름을 붙여 둔 버전과 복원 단추](images/guide/editor-history.png)
...
![공유 대화상자 — 사용자·부서·역할을 더하는 자리, 액세스 권한 목록, 일반 액세스 범위, 소유자 전용 정책과 소유권 이전](images/guide/editor-share.png)
...
3. 막대, 누적 막대, 선, 영역, 누적 영역, 혼합(막대+선), 원형, 분산, 히스토그램, 일정표, 폭포 중 유형을 선택하고 원본 시트·범위, 제목, 범례와 축 제목을 설정합니다.
4. **폭포** 는 열을 앞에서부터 **이름 · 값 · 합계 여부** 로 읽습니다. 막대는 앞줄까지의 누계에서 시작해 값만큼 자라므로, 매출에서 원가와 판관비를 빼 영업이익에 이르는 길이 한눈에 보입니다. 늘어난 것과 줄어든 것은 **색으로 갈라** 그립니다. 셋째 칸에 `TRUE` 나 `합계` 를 적은 줄은 **바닥부터** 그립니다 — 그런 줄은 증감이 아니라 앞의 것을 합쳐 놓은 자리이기 때문입니다. 표에 적힌 합계가 앞의 값을 더한 것과 달라도 **표에 적힌 값을 그립니다.** 그림이 표와 어긋나면 어느 쪽이 맞는지 알 수 없기 때문입니다. 엑셀에는 같은 그림이 없어 XLSX로 내보낼 때는 일정표와 함께 빠지고, 몇 개가 빠졌는지 알려 줍니다.
5. 생성된 차트의 머리글을 드래그해 위치를 옮기고 오른쪽 아래 핸들로 크기를 조절합니다. 내보내기 버튼으로 SVG 또는 PNG 파일을 저장할 수 있습니다.

![차트 패널 — 오른쪽 목록에 원본 범위와 버전이 적힌 차트 카드, 시트 위에 그려진 막대 차트](images/guide/editor-chart.png)

**일정표** 는 열을 앞에서부터 **이름 · 시작 · 끝** 으로 읽어 가로 막대로 그립니다. 끝을 적지 않으면 그 날 하루짜리 이정표로 봅니다. 끝이 시작보다 앞이면 거꾸로 적은 것으로 보고 바로잡아 그립니다 — 그대로 두면 막대가 뒤로 자라 읽을 수 없습니다. 날짜는 `=DATE(2026,1,5)` 같은 글자도, 파일에서 들어온 날 수도 함께 읽습니다. 일감이 많아 한 줄이 6픽셀보다 얇아지면 그릴 수 있는 만큼만 그리고 **몇 개가 남았는지 적습니다** — 그림 밖으로 넘쳐 흐르는 것보다 낫습니다. 일정표는 엑셀에 같은 것이 없어 XLSX 로 내보낼 때는 빠집니다 — **몇 개가 빠졌는지 내보낸 뒤에 알려 드립니다.** 시트의 자료는 그대로 나가므로 엑셀에서 다시 그릴 수 있습니다.
...
![AI 도우미 패널 — 관리자가 사내 LLM Gateway 를 켜지 않은 상태에서는 이렇게 안내한다](images/guide/editor-ai.png)
...
6. 적용 직후 **Undo**를 누르면 Agent가 바꾼 셀·차트·피벗·조건부 서식 규칙·입력 규칙·필터 보기·표·정렬·신규 보고서 시트를 ChangeSet 단위로 되돌릴 수 있습니다. 적용 뒤 다른 변경이 생긴 경우에는 그 변경을 보호하기 위해 롤백을 거부합니다.
6-0. **"이 범위를 표로 만들어줘" 라고 하면 이름 있는 표를 만듭니다.** "A1:C20을 매출표로 바꿔줘", "선택한 곳을 테이블로 변환해줘" 처럼 말하면 됩니다. 표가 생기면 `=SUM(매출표[금액])` 처럼 열 이름으로 가리킬 수 있고, 행과 열이 움직여도 그 수식이 그대로 맞습니다. "표 서식을 입혀줘"(색 칠하기)나 "피벗 테이블 만들어줘"(피벗)와는 갈라서 알아듣습니다.
...
**데이터 › 가정 분석 › 목표값 찾기…** 를 열고 세 가지를 적습니다.
...
### 19.1 테마 및 개인화
- 프로필 메뉴의 **개인 환경설정** 또는 `/preferences` 로 들어갑니다.
- **화면 테마** 는 라이트 · 다크 · 시스템 설정 중에서 고릅니다.
- **언어 및 로케일** 은 날짜와 숫자 표시 방식을, **화면 밀도** 는 도구와 목록의 간격을 정합니다.
- 고친 뒤 **개인 설정 저장** 을 누릅니다. 이 설정은 지금 계정에만 적용되며 시스템 설정과 별개입니다.

![개인 환경설정 — 화면 테마, 언어 및 로케일, 화면 밀도](images/guide/preferences.png)

### 19.2 개인 API 키 (Personal API Keys) 발급
파이썬 스크립트나 커스텀 애플리케이션에서 kanpic 데이터에 프로그램 방식으로 접근할 수 있습니다.
**API 키** 탭에서 **+ 새 키** 를 누르고 이름과 scope, 만료를 정하면 됩니다.

![개인 API 키 목록 — 키마다 앞자리와 scope, 생성·만료·최근 사용이 적히고 수정·회전·폐기를 할 수 있다](images/guide/preferences-keys.png)

목록에는 키의 앞자리(`kp_live_…`)만 보입니다. 원문은 만든 직후 한 번만 보여 주므로
그때 옮겨 적어 두세요.

```python
import requests

# 개인 API 키 설정 (원문은 발급 직후 한 번만 표시됩니다)
API_KEY = "kp_live_0000000000000000000000000000"
...
### 20.1 막혔을 때 — 화면에 나오는 말 그대로

| 화면에 나오는 말 | 뜻과 할 일 |
| --- | --- |
| `CSV must be UTF-8 encoded` | 올린 파일이 무엇으로 적혔는지 밝히지 않았고 UTF-8 도 아닙니다(EUC-KR 로 저장한 파일이 대개 그렇습니다). 엑셀에서 **CSV UTF-8** 로 다시 저장해 올리세요. 파일 앞에 표시(BOM)가 있는 UTF-8·UTF-16 은 그대로 읽습니다. |
| `import exceeds one million non-empty cells` · `import exceeds one million stored cells` | 한 번에 들일 수 있는 셀 수를 넘었습니다. 시트를 나누어 올리세요. |
| **AI가 아직 비활성화되어 있습니다.** | 이 배포에서 AI 를 아직 켜지 않았습니다. 관리자에게 사내 LLM Gateway 설정을 요청하세요(관리자 가이드의 AI Gateway 절). |
| **저장 대기 중인 변경을 먼저 서버에 반영해야 합니다.** | 아직 서버에 올라가지 않은 편집이 있습니다. 연결을 확인하고 `Ctrl/⌘+S` 로 반영한 뒤 다시 하세요. |
| **시트 레이아웃은 서버에 연결된 상태에서 변경할 수 있습니다.** | 오프라인입니다. 값 입력은 대기열에 쌓이지만 시트 추가·삭제 같은 구조 변경은 연결된 뒤에 할 수 있습니다. |
| **두 개 이상의 셀을 선택해 병합하세요.** · **선택 범위가 기존 병합 셀과 겹칩니다. 먼저 병합을 해제하세요.** | 병합할 범위를 다시 고르거나, 겹친 병합을 먼저 풉니다. |
| 셀에 `#REF!` | 수식이 가리키던 셀·시트가 사라졌습니다. 차트라면 원본 범위가 지워진 것이니 원본을 되살리거나 차트의 범위를 다시 지정합니다. |
| 셀에 `#VALUE!` · `#NUM!` · `#DIV/0!` · `#NAME?` | 각각 값의 종류가 맞지 않음 · 계산할 수 없는 수 · 0으로 나눔 · 모르는 이름입니다. 셀을 고르면 수식 입력줄에서 무엇을 쓴 것인지 볼 수 있고, 오른쪽 클릭 메뉴의 도움말이 어디가 문제인지 짚어 줍니다. |
| 워크북을 열 때 **권한이 없습니다** 류의 안내 | 공유받지 못한 워크북입니다. 소유자에게 액세스를 요청하면 소유자 화면에 요청이 뜹니다(8.6). |

여기서 풀리지 않는 것 — 로그인 자체가 되지 않거나, 관리자 콘솔 설정이 필요한 일(AI·메일·외부 호출·프레젠테이션) — 은
관리자에게 넘깁니다. 문의할 때 로그인 화면 왼쪽 아래의 버전(`kanpic v0.242.0 · <빌드>`)을 함께 알려 주세요.
...
| 말 | 뜻 |
| --- | --- |
| 워크북 | 시트 여러 장을 담는 파일 한 개에 해당합니다. 공유·권한·버전은 워크북 단위입니다. |
| 시트 | 워크북 안의 표 한 장. 아래쪽 탭으로 오갑니다. |
| 이름 상자 | 수식 입력줄 왼쪽의 칸. `C7`, `A1:D9`, 이름 범위를 적어 그 자리로 갑니다. |
| 이름 범위 | 범위에 붙인 이름. 수식에서 `=SUM(매출)` 처럼 씁니다. |
| 표 | 범위에 이름을 붙여 열 이름으로 가리키게 한 것. `=SUM(매출표[금액])`. |
| 필터 보기 | 남의 화면을 바꾸지 않고 나만 걸러 보는 방식. |
| 버전 | 저장될 때마다 올라가는 번호. 이름을 붙여 두면 그 시점으로 복원할 수 있습니다. |
| Outbox | 연결이 끊겼을 때 입력을 담아 두는 브라우저 안의 대기열. 다시 연결되면 자동으로 보냅니다. |
| 링크 액세스 | 링크를 가진 사람에게 열어 주는 범위(제한됨 · 조직 내 모든 사용자 · 링크가 있는 모든 사용자). |
| scope | API 키가 할 수 있는 일의 범위(`range.read`, `mcp.use` 등). |
| MCP | 에이전트가 kanpic 을 도구로 부르는 규약. `/mcp` 주소로 제공됩니다. |
