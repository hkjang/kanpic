...
**고르는 자리에서 몇 장으로 나뉘는지 바로 보여 줍니다.** 방향이나 여백을 바꾸면 그 자리에서 장수가 바뀌고, 어느 열이 어느 장에 가는지도 함께 적힙니다 — 종이가 나온 뒤에 아는 것은 늦습니다. 이 셈은 인쇄가 실제로 쓰는 것과 **같은 코드** 입니다. 따로 세면 "한 장에 들어갑니다" 라고 보여 주고 두 장을 뱉는 일이 생기고, 그러면 미리보기는 없느니만 못합니다. 세로 길이는 종이 크기와 행 높이에 따라 브라우저가 정하므로 장수를 적지 않습니다 — 모르는 것을 아는 척하지 않습니다.
