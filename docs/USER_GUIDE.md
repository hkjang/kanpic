...
**셀 서식은 그대로 찍힙니다.** 배경색, 글자색, 굵게·기울임·밑줄, 글꼴과 크기, 맞춤, 테두리가 화면에서 본 대로 종이에 나옵니다. 조건부 서식으로 칠해진 배경색도 함께 나가지만, 데이터 막대와 아이콘 집합은 화면에서만 그려집니다.
...
## 18. 개인화 설정 및 개인 API 키 (`/preferences`)

### 18.1 테마 및 개인화
- `/preferences` 페이지에서 화면 테마(Light/Dark/System)를 설정할 수 있습니다.

### 18.2 개인 API 키 (Personal API Keys) 발급
