...
- **템플릿으로 시작**: 빠른 시작 영역의 추천 템플릿을 누르거나 **템플릿 갤러리** 에서 30개 템플릿을 분류·검색해 고릅니다. 템플릿은 표 머리글, 예시 데이터, 계산되는 수식, 통화·백분율 서식, 열 너비, 머리글 고정까지 갖춘 상태로 열립니다.
...
편집기의 **도움말 › 함수 목록** 은 아래 표와 같은 내용을 서버에서 직접 받아 보여 줍니다. 목록에 없는 함수는 `#NAME?` 오류를 반환합니다. **삽입 › 함수** 에서 자주 쓰는 집계 함수를 선택하면 선택한 범위를 인수로 채운 수식이 바로 입력됩니다.
...
- `=AND(조건1, 조건2, ...)` / `=OR(조건1, 조건2, ...)` / `=NOT(조건)`: 다중 논리 연산을 수행합니다.
...
- `=FILTER`, `=SORT`: 조건에 맞는 행만 남기거나 정렬한 결과를 배열로 반환합니다.
