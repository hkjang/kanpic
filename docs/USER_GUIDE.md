...
**편집이 다른 곳의 수식을 깨뜨리면 그 자리에서 알려 줍니다.** 행이나 열을 지워 다른 셀·다른 시트의 수식이 `#REF!` 가 되면 왼쪽 아래에 몇 곳이 깨졌는지와 첫 번째 위치가 나타납니다. **보기** 를 누르면 그 셀로 이동합니다. 편집을 막지 않는 안내이므로 그대로 작업을 이어가도 되고, 잘못 지웠다면 `Ctrl/⌘ + Z` 로 되돌리면 됩니다.
