...
**이름 정의도 양방향으로 옮겨집니다.** kanpic에서 만든 이름 있는 범위는 엑셀의 이름 정의로 나가고, 엑셀 파일의 이름 정의도 읽어 옵니다. 한글 이름도 그대로 쓰이며, `=SUM(단가)` 같은 수식은 가져온 직후 첫 계산에서 바로 값을 냅니다. 시트 하나에만 적용되는 이름, 인쇄 영역(`_xlnm.Print_Area`), 상수나 수식을 가리키는 이름, 다른 파일을 가리키는 이름은 kanpic에 대응이 없어 가져오지 않고 미리보기에서 몇 개인지 알려 줍니다.
