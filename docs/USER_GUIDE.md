...
첫 행은 항목 이름, 첫 열은 줄 이름표로 봅니다. 어느 시트에만 있는 이름표도 결과에 들어가고, **없는 칸은 비워 둡니다** — 없는 것은 0이 아니라 세지 않은 것이고, 0을 적으면 실적이 0원이었다는 뜻이 됩니다. 무엇이 숫자인지는 `=SUM` 과 같은 규칙으로 보므로 `"1,234"` 처럼 글자로 담긴 숫자는 빠지고, **몇 칸이 빠졌는지 세어 알려 줍니다.** 병합된 셀이 있는 시트도 이름을 짚어 줍니다 — 병합된 이름표 열은 빈 이름표 행을 만들어 그 행이 통째로 빠지기 때문입니다. 한 번에 시트 열두 개까지 읽고, 그보다 많으면 몇 개만 읽었는지 말합니다. 결과는 새 시트에만 적습니다.
