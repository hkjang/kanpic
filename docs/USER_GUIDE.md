...
**표는 격자 위에 테두리와 이름으로 보이고, 색을 고를 수 있습니다.** 머리글 줄에는 옅은 색이 깔리고, 표 이름이 왼쪽 위에 붙습니다. 파랑·초록·주황·빨강 가운데 고른 색으로 그리며, **그 색이 XLSX 로 내보낼 때 엑셀의 표 서식이 됩니다** — 화면과 파일이 다른 색이면 어느 쪽이 맞는지 알 수 없기 때문입니다. 표가 여럿일 때 어디까지가 어느 표인지 한눈에 갈립니다. 칸에 색을 칠해 두는 것이 아니라 그려 주는 것이므로, 표가 행을 따라 움직이면 표시도 함께 움직입니다.
