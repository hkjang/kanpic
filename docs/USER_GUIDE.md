...
2. **수동 실행** 또는 **셀 변경 시**를 선택합니다. 셀 변경 트리거는 감시할 시트와 A1 범위를 함께 지정합니다.
...
6. 실행 이력에서 성공·실패·Undo 상태를 확인합니다. 성공한 실행은 후속 셀 변경과 충돌하지 않을 때 **Undo**할 수 있습니다.
...
현재 지원하는 트리거는 수동 실행과 셀 변경이며 작업은 값 설정, 수식 설정, 내용 지우기입니다. 스케줄, 웹훅, 알림, AI와 MCP 연쇄 실행은 이후 단계에서 추가됩니다. 자동화가 자신의 트리거 범위를 다시 변경하더라도 연쇄 자동화를 자동으로 발생시키지 않습니다.
