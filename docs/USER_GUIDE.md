...
**두 목록 비교(대사)** 는 두 범위를 키 열로 견주어 어느 쪽에만 있는 항목인지 찾습니다. 비교할 범위는 `장부!A1:C100` 처럼 시트 이름을 붙여 적습니다 — 대사는 대개 시트 두 개를 견주는 일이기 때문입니다. **한쪽이 `"1,000"` 글자이고 다른 쪽이 `1000` 숫자여도 같은 항목으로 맞춥니다.** 은행에서 받은 내역과 장부는 거의 항상 이렇게 다르고, 글자 그대로 견주면 하나도 맞지 않습니다. 같은 키가 한쪽에서 거듭 나오면 그것도 함께 알려 줍니다 — 대사에서는 그 자체가 발견입니다. 키 칸이 비어 견주지 못한 행의 수도 적습니다. 조용히 빼면 "다 맞았다" 는 대사표가 나오고 수는 맞지 않습니다. 결과는 **새 시트에만** 적고 원래 자료는 건드리지 않습니다. **행이 움직이는 정리(중복 항목 삭제·부분합 제거)와 텍스트를 열로 분할은 병합된 셀을 만나면 멈추고 어느 칸이 걸렸는지 알려 줍니다.** 정렬과 같은 규칙입니다 — 병합을 먼저 해제한 뒤에 하세요. 공백 제거와 텍스트로 저장된 숫자는 제자리에서 고치므로 병합과 상관없이 됩니다.
...
- 쉼표나 통화 기호가 붙어 **글자로 담긴 숫자** 는 **데이터 › 데이터 정리 › 텍스트로 저장된 숫자…** 로 고칩니다. `1,234` `₩5,000` `(500)` `50%` 를 알아보고 바꿔 줍니다. 괄호는 음수로, 백분율은 100으로 나눕니다. **보이던 모양은 그대로 둡니다** — `₩` 와 `%` 는 표시 형식으로 옮기므로, 칸에 담기는 값만 숫자로 바뀌고 화면은 달라지지 않습니다. 쉼표가 세 자리마다 찍혀 있지 않으면 숫자로 보지 않습니다 — 억지로 바꾸면 사람이 적은 것과 다른 값이 들어갑니다.
