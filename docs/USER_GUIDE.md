...
- `=TEXT(값, 서식)`: 숫자나 날짜를 `#,##0`, `0.0%`, `yyyy-mm-dd` 같은 서식의 글자로 바꿉니다. 서식은 `;` 로 구역을 나눠 **양수 ; 음수 ; 0** 을 따로 적을 수 있습니다. `#,##0;(#,##0);"-"` 은 0 을 `-` 로 적고, 구역을 비워 두면(`0;;`) 그 값은 아무것도 적지 않습니다. 자리 기호 **뒤** 에 붙인 쉼표는 자릿점이 아니라 천 단위로 줄여 적으라는 뜻이라 `#,##0,,` 은 백만 단위(`1234567` → `1`), `0.0,,"백만"` 은 `1.2백만` 입니다. 칸의 표시 형식도 같은 규칙으로 그립니다.
