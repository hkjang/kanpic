...
| 데이터 | 정렬(선택 열 기준 A→Z·Z→A, 범위 정렬), 두 목록 비교, 필터 보기, 슬라이서 추가, 데이터 연결, 데이터 검증, 피벗 테이블, 이름 범위, 데이터 정리(중복 항목 삭제·공백 제거·텍스트로 저장된 숫자·병합 해제하고 채우기·부분합 제거, 미리보기), 텍스트를 열로 분할(미리보기), 부분합, 행·열 숨기기, 워크북 검색 |
