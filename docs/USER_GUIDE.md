...
- 쉼표나 통화 기호가 붙어 **글자로 담긴 숫자** 는 **데이터 › 데이터 정리 › 텍스트로 저장된 숫자…** 로 고칩니다. `1,234` `₩5,000` `(500)` `50%` 를 알아보고 바꿔 줍니다. 괄호는 음수로, 백분율은 100으로 나눕니다. 열다섯 자리를 넘는 숫자는 바꾸지 않습니다 — 그런 것은 금액이 아니라 계좌번호이고, 바꾸면 뒤가 뭉개집니다. **보이던 모양은 그대로 둡니다** — `₩` 와 `%` 는 표시 형식으로 옮기므로, 칸에 담기는 값만 숫자로 바뀌고 화면은 달라지지 않습니다. 쉼표가 세 자리마다 찍혀 있지 않으면 숫자로 보지 않습니다 — 억지로 바꾸면 사람이 적은 것과 다른 값이 들어갑니다.
