...
HTML이 없는 곳(메모장, 터미널)에서 붙여넣을 때도 보이던 글자를 숫자로 읽습니다. `1,234`, `98,000원`, `(1,234)` 는 숫자로 들어오고 보이던 모습은 표시 형식으로 남습니다. 쉼표가 세 자리씩 끊지 않는 `1,2` 처럼 자릿수 구분으로 볼 수 없는 값, 전화번호, 날짜는 글자 그대로 둡니다. `007` 처럼 앞자리 0이 뜻을 갖는 우편번호·사번과 열여섯 자리가 넘는 계좌·전표 번호도 글자로 둡니다 — 숫자로 바꾸면 앞의 0이 사라지거나 뒷자리가 뭉개져 되돌릴 수 없기 때문이며, 같은 표를 CSV·XLSX로 가져올 때와 같은 규칙입니다. (셀에 직접 쳐 넣은 `007` 은 스프레드시트가 늘 하던 대로 숫자 7이 됩니다. 밖에서 옮겨 온 표만 적힌 대로 지킵니다.)
...
- 쉼표나 통화 기호가 붙어 **글자로 담긴 숫자** 는 **데이터 › 데이터 정리 › 텍스트로 저장된 숫자…** 로 고칩니다. `1,234` `₩5,000` `(500)` `50%` 를 알아보고 바꿔 줍니다. 괄호는 음수로, 백분율은 100으로 나눕니다. 열다섯 자리를 넘는 숫자는 바꾸지 않습니다 — 그런 것은 금액이 아니라 계좌번호이고, 바꾸면 뒤가 뭉개집니다. 통화 기호와 뒤에 붙은 단위(`원`·`USD`)도 표시 형식으로 옮겨 그대로 보입니다. CSV로 가져올 때는 쉼표나 통화 기호가 붙은 값이 글자로 들어옵니다. 가져오기 미리보기가 **몇 칸인지 세어 알려 주므로**, 가져온 뒤 이 정리로 한 번에 고치면 됩니다. 열다섯 자리를 넘는 수는 CSV에서도 글자로 둡니다 — 실수로 읽으면 파일에 적힌 것과 다른 값이 칸에 들어가고 되돌릴 수 없습니다.
