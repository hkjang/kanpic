...
- 쉼표나 통화 기호가 붙어 **글자로 담긴 숫자** 는 **데이터 › 데이터 정리 › 텍스트로 저장된 숫자…** 로 고칩니다. `1,234` `₩5,000` `(500)` `50%` 를 알아보고 바꿔 줍니다. 괄호는 음수로, 백분율은 100으로 나눕니다. 쉼표가 세 자리마다 찍혀 있지 않으면 숫자로 보지 않습니다 — 억지로 바꾸면 사람이 적은 것과 다른 값이 들어갑니다.
