...
func (r *MemoryRepository) recalculateAllLocked(state *workbookState) error {
	...
	expanded, _, _, err := recalculateCellInputs(state.sheets, state.cells, currentSheetID, nil, true, nameContext{Ranges: formulaNamedRanges(r.namedRangesForWorkbookLocked(state.workbook.ID)), Functions: r.namedFunctionDefinitionsLocked(state.workbook.ID), Imports: r.importsForLocked(state.workbook.ID, state.cells, nil)})
	if err != nil {
		return err
	}
	now := r.now()
