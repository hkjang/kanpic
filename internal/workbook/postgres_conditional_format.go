...
const conditionalFormatColumns = `c.id::text,w.id::text,w.version,c.sheet_id::text,c.idempotency_key,c.name,c.cell_range,c.rule_type,c.operator,c.formula,c.value,c.value2,c.style,c.min_color,c.mid_color,c.max_color,c.bar_color,c.icon_style,c.icon_reverse,c.priority,c.stop_if_true,c.revision,c.created_by,c.updated_by,c.created_at,c.updated_at`
...
func (r *PostgresRepository) UpdateConditionalFormat(ctx context.Context, id, actor string, input UpdateConditionalFormatInput) (ConditionalFormat, error) {
	...
	command, err := tx.Exec(ctx, `UPDATE conditional_formats SET name=$2,cell_range=$3,rule_type=$4,operator=$5,formula=$19,value=$6,value2=$7,style=$8,min_color=$9,mid_color=$10,max_color=$11,bar_color=$12,priority=$13,stop_if_true=$14,revision=$15,updated_by=$16,updated_at=$17,icon_style=$20,icon_reverse=$21 WHERE id=$1 AND revision=$18`, id, updated.Name, updated.Range, updated.RuleType, updated.Operator, nullableJSON(updated.Value), nullableJSON(updated.Value2), nullableJSON(updated.Style), updated.MinColor, updated.MidColor, updated.MaxColor, updated.BarColor, updated.Priority, updated.StopIfTrue, updated.Revision, actor, now, current.Revision, updated.Formula, updated.IconStyle, updated.IconReverse)
	if err != nil {
		return ConditionalFormat{}, err
	}
