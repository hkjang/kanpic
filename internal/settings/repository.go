...
var defaults = []Setting{
	...
	{Key: "auth.oidc.auto_login", Value: json.RawMessage(`false`), ValueType: "boolean", Description: "자동 로그인(Silent SSO). Keycloak 에 이미 로그인한 사람은 로그인 화면 없이 바로 들여보냄"},
	...
	{Key: "external.enabled", Value: json.RawMessage(`false`), ValueType: "boolean", Description: "수식의 외부 호출(WEBSERVICE, IMPORTDATA) 사용. 켜도 허용 호스트에 적은 곳만 부릅니다"},
	{Key: "external.allowed_hosts", Value: json.RawMessage(`[]`), ValueType: "string_list", Description: "외부 호출을 허용할 호스트. api.example.com 또는 *.example.com. 비어 있으면 아무 데도 부르지 않습니다"},
