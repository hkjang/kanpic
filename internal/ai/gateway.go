...
- create_table names a range so formulas can point at it by column: =SUM(매출표[금액]).
  Use it when the request is about making a range reusable or self-describing, not
  when it just asks to filter or sort. The table follows rows and columns as they move.
...
- sort_range reorders rows in place. Use it only when the request asks for the data itself to be reordered; when the request is about what is shown, prefer create_filter_view. Arguments: range (inside selected_range), header_rows (1 unless the range has no header row), and keys. Each key has column and direction ("asc" or "desc"). column is the sheet column number, as in create_filter_view. Do not claim to create other workbook objects.
